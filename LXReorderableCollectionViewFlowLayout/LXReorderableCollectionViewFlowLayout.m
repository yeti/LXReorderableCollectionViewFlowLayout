...
- (void)invalidateLayoutIfNecessary {
  ...
  if (newIndexPath.row == [self.collectionView numberOfItemsInSection:0] - 1) {
    _coveringLast = YES;
    return;
  }
...
- (void)handleLongPressGesture:(UILongPressGestureRecognizer *)gestureRecognizer {
  switch(gestureRecognizer.state) {
    ...
    case UIGestureRecognizerStateEnded: {
      ...
      if ([self.delegate respondsToSelector:@selector(collectionView:layout:didEndDraggingItemOverLastIndexPath:)]) {
        if (_coveringLast) {
          [self.delegate collectionView:self.collectionView layout:self didEndDraggingItemOverLastIndexPath:currentIndexPath];
        }
      }
